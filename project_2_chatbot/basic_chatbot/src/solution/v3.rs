...
impl ChatbotV3 {
    ...
    let user_session = self.sessions.get(&username);
    match user_session {
        None => {
            return Vec::new();
        }
        Some(chat_session) => {
            ...
            let all_messages = raw_session.history();
            let messages_as_strings: Vec<String> = all_messages
                .iter()
                .map(|message| format!("{:?}", message))
                .collect();

            return messages_as_strings;
        }
    }
}
